...
class MarzbanAPI:
    ...
    async def get_token(self):
        try:
            ...
            logger.info(f"Token almak ucin URL: {url}")
            ...
            logger.info(f"Token alindi: {bool(self.token)}")
            return self.token
    ...
    async def create_user(self, username, data_limit_gb=125, expire_days=30):
        ...
        try:
            ...
            logger.info(f"User doretmek ucin URL: {url}")
            response = await self.client.post(
                url,
                json=user_data,
            ...
            if response.status_code == 401:
                logger.info("Token gutardi, taze alinyar...")
                await self.get_token()
                response = await self.client.post(
                    url,
                    json=user_data,
            ...
            logger.info(f"User doretme response status: {response.status_code}")
            response.raise_for_status()
    ...
    async def get_user(self, username):
        ...
        try:
            ...
            logger.info(f"User getirmek ucin URL: {url}")
            response = await self.client.get(
                url,
                headers={"Authorization": f"Bearer {self.token}"}
            )
            logger.info(f"User getirme response status: {response.status_code}")
            response.raise_for_status()
            return response.json()
        except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user.id):
        await update.message.reply_text("Bu bota girish rugsady yok.")
        return

    welcome_text = (
        f"Salam {user.first_name}!\n\n"
        f"Marzban VPN Boty\n\n"
        f"Bu bot bilen awtomatik usulda VPN ulanyjylary doredip bilersiniz.\n\n"
        f"Buyruklar:\n"
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Status yalnyshlygy: {e}")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update.effective_user.id):
        await query.edit_message_text("Bu bota girish rugsady yok.")
        return
